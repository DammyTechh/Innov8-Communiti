...
| Job | Checks |
|---|---|
| Typecheck, lint, build, migration drift | `tsc`, type-aware ESLint, compile, schema and migration in sync, critical npm audit |
| Database + end-to-end tests | Postgres 16, migration applied twice, 359 e2e checks against the compiled API with pool size 1, cron auth, OpenAPI validation, no errors in the log |
